...
class Fetch implements Transport {
  ...
  controller: AbortController | undefined =
    (self as any).AbortController && new AbortController();
  ...
  pump(readerArg: ReadableStreamDefaultReader<Uint8Array>, res: Response) {
  ...
  send(msgBytes: Uint8Array) {
    fetch(this.options.url, {
      ...
      .then((res: Response) => {
        ...
        console.log("after");
        this.options.onHeaders(res.headers as any, res.status);
        if (res.body) {
